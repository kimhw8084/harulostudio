...
## Follow-up candidate from moved main

The original remediation was committed as `12521f11b2efd5bfc34badaa7f5fc8778ca02825` and subsequently pushed to `main` at the user's request. This follow-up starts from that exact SHA on branch `golden-ui-remediation-review`; the original `4512c2f80bcc6ab222055ba7d290650bb1b3e990` baseline was not reset or overwritten. The source diff from the original baseline was inspected before further edits. The original Linux screenshots remain defect evidence, and `work/golden-ui-candidate/` is the current-main rendered baseline for this review.

| Evidence and route/state | Failure class | Rule | Suspected shared root and remediation | Final verification path |
| --- | --- | --- | --- | --- |
| `work/golden-ui-candidate/screenshots/home-genesis-open-mobile.png`; `/` at 390px, Genesis open | P2: The fifth concept tab is clipped at the right edge of the horizontal selector. | JOIN-06, ACC-03, LAY-03 | The mobile tablist inherited a single-row horizontal scroll pattern. Use a two-row, three-column selector so every concept and its 44px target are immediately visible. | New 320/390px rendered tab geometry contract; `work/golden-ui-review-candidate/screenshots/home-genesis-open-mobile.png`. |
| `work/golden-ui-candidate/screenshots/sori-desktop.png` and other product details | P2: Large slogan-like feature heading and oversized Details and related headings compete with the product specimen. | TASK-01, VIS-03, VIS-04 | Shared product-detail template promotes secondary editorial sections. Use functional `What it does` and `Details` headings and a smaller related-software section. | Five desktop/mobile/dark detail captures in the new evidence pack. |
| `work/golden-ui-candidate/screenshots/studio-desktop.png`; `/studio` name-meaning section | P2: The studio description repeats the page introduction immediately before the approved literal and poetic explanation. | TASK-01, VIS-04 | A `full` variant adds redundant copy to the shared Studio story. Remove the variant and retain one literal statement, one visual, and one approved close. | Studio desktop/mobile/dark captures in the new evidence pack. |

Follow-up correctness gates with Node 22: `npm ci`, typecheck, lint, brand verification, 3 unit tests, and production build passed. Functional Playwright passed 94 with 14 scoped skips; Axe passed 40 with 2 scoped skips; motion contracts passed 15 across Chromium, reduced-motion Chromium, and WebKit. Synthetic mobile performance measured 172,287 initial encoded JavaScript bytes, 189,328 total gzip chunk bytes, 64ms worst local LCP, and 0.00161 maximum CLS. The 17 old visual checkpoint tests still differ, as expected: their snapshots have not been accepted or replaced. A separate follow-up evidence pack is generated at `work/golden-ui-review-candidate/` for independent visual review.
