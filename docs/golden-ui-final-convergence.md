...
Repeated Chromium inspection then exposed a separate empty Accessibility reading list while the named product groups remained visible. The first implementation populated that list from an effect-time DOM and layout read, so the selected tab and registered group names could disagree. Classification: **P2 / semantic registration lifecycle**. Owner: `SoftwareXRay`. The final list uses an explicit per-product registry of group labels and actual element IDs; the specimen keeps the matching semantic groups. Repeated tests check both sides for every concept.
