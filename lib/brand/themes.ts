...
export const themeBootstrap = `try{const t=localStorage.getItem('harulo-theme');const m=t==='evening'?'dark':t==='daylight'?'light':t;const v=m==='light'||m==='dark'?m:matchMedia('(prefers-color-scheme:dark)').matches?'dark':'light';document.documentElement.dataset.theme=v;if(t&&t!==m)localStorage.setItem('harulo-theme',v);document.documentElement.dataset.motion=matchMedia('(prefers-reduced-motion:reduce)').matches?'paused':'running'}catch(e){}`;
