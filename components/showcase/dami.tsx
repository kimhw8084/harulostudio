...
export function DamiSpecimen({ mode = "detail" }: { mode?: "genesis" | "detail" }) {
  ...
  return (
    <ShowcaseShell name="Dami" kind="dami" mode={mode}>
      ...
      <label className="showcase-select-label" htmlFor="dami-allocation">Editable allocation in US dollars</label>
      <div className="dami-money-input"><span aria-hidden="true">$</span><input id="dami-allocation" type="number" min="0" max={centsToDollars(data.budget)} step="1" value={centsToDollars(allocation)} onChange={(event) => setAllocation(clampInt(dollarsToCents(Number(event.target.value) || 0), 0, data.budget))} /></div></div>
      {mode === "detail" && <><div id="dami-inspection-ledger" role="group" aria-label="Ledger" data-inspection-group="Ledger"><div className="showcase-table-scroll" role="region" aria-label="Monthly ledger" tabIndex={0}><table className="showcase-table"><caption>{month} sample categories and recurring items</caption><thead><tr><th scope="col">Category</th><th scope="col">Amount</th></tr></thead><tbody>{rows.map(([name, value]) => <tr key={name} data-selected={name === category}><th scope="row">{name}</th><td>{formatMoney(value)}</td></tr>)}{data.recurring.map((item) => <tr key={item.label}><th scope="row">Recurring · {item.label}</th><td>{formatMoney(item.amount)}</td></tr>)}</tbody></table></div></div>
      <div id="dami-inspection-chart" role="group" aria-label="Chart" data-inspection-group="Chart"><div className="dami-chart" role="img" aria-label={`Sample category spending for ${month}`}>{rows.map(([name, value]) => <div key={name}><span>{name}</span><i style={{ width: `${Math.min(100, (value / Math.max(...Object.values(data.categories))) * 100)}%` }} /><strong>{formatMoney(value)}</strong></div>)}</div></div>
