...
export function DamiSpecimen({ mode = "detail" }: { mode?: "genesis" | "detail" }) {
  ...
  return (
    <ShowcaseShell name="Dami" kind="dami" mode={mode}>
      ...
      {mode === "detail" && <><table className="showcase-table"><caption>{month} sample categories and recurring items</caption><thead><tr><th scope="col">Category</th><th scope="col">Amount</th></tr></thead><tbody>{rows.map(([name, value]) => <tr key={name} data-selected={name === category}><th scope="row">{name}</th><td>{formatMoney(value)}</td></tr>)}{data.recurring.map((item) => <tr key={item.label}><th scope="row">Recurring · {item.label}</th><td>{formatMoney(item.amount)}</td></tr>)}</tbody></table>
      <div className="dami-chart" role="img" aria-label={`Sample category spending for ${month}`}>{rows.map(([name, value]) => <div key={name}><span>{name}</span><i style={{ width: `${Math.min(100, (value / Math.max(...Object.values(data.categories))) * 100)}%` }} /><strong>{formatMoney(value)}</strong></div>)}</div>
