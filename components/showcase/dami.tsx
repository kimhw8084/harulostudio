...
const monthData: Record<Month, { budget: number; categories: Record<string, number>; recurring: { label: string; amount: number }[] }> = {
  September: { budget: 120000, categories: { Food: 28400, Travel: 12200, Home: 16800 }, recurring: [{ label: "Rent", amount: 70000 }, { label: "Phone", amount: 4200 }] },
  August: { budget: 120000, categories: { Food: 31200, Home: 19400, Other: 8600 }, recurring: [{ label: "Rent", amount: 70000 }, { label: "Phone", amount: 4200 }, { label: "Books", amount: 2200 }] },
};
...
export function DamiSpecimen() {
  ...
  const allocated = Math.max(0, allocation);
  ...
  return (
    <ShowcaseShell name="Dami">
      ...
      <input id="dami-allocation" type="number" min="0" max={data.budget} step="100" value={allocation / 100} onChange={(event) => setAllocation(clampInt(Number(event.target.value) * 100, 0, data.budget))} />
